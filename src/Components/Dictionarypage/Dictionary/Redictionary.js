...
// import { setWord, setDefinition, setError } from './RedictionaryActions';
// import './Redictionary.css'
//
//
// function Redictionary() {
...
//
//     // Использование хуков useSelector и useDispatch для доступа к состоянию Redux и диспетчеру
...
//             dispatch(setDefinition(response.data));
//             dispatch(setError(''));
//         } catch (err) {
//             dispatch(setDefinition(null));
//             dispatch(setError('Word not found. Please try again.'));
//         }
//     };
//
//     const saveWord = () => {
...
//                 dispatch(setWord('')); // Обнуляем слово после сохранения
//                 dispatch(setDefinition(null)); // Обнуляем определение после сохранения
//             }
//         }
//     };
//
...
//             <h1 className="dictionary-heading">Redictionary</h1>
...
function Redictionary() {
    ...
    // Использование хуков useSelector и useDispatch для доступа к состоянию Redux и диспетчеру
    const word = useSelector(state => state.redictionary.word);
    const definition = useSelector(state => state.redictionary.definition);
    const error = useSelector(state => state.redictionary.error);
    const dispatch = useDispatch();
    ...
    const openDefinitionModal = (definition) => {
        ...
        const contentWidth = definition.word.length * 12;
        const contentHeight = definition.meanings.reduce((acc, meaning) => {
            return acc + meaning.definitions.reduce((defAcc, def) => {
                return defAcc + def.definition.length;
            }, 0);
        ...
        setModalStyle({
            width: `${modalWidth}px`,
            height: `${modalHeight}px`,
            borderRadius: '10px' // Здесь устанавливаем радиус закругления
        });
    };
